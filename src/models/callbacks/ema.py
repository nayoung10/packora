...
class EMA(Callback):
    ...
    def __init__(
        ...
        warm_start: bool, optional (default=True)
            Ramp up the EMA decay from a lower value early in training.
        initial_weights: dict[str, Any], optional
            Explicit weights used to initialize a new EMA trajectory.
    ...
    def on_train_start(self, trainer: Trainer, pl_module: LightningModule) -> None:  # noqa: ARG002
        ...
        if not self.ema_initialized:
            initial_weights = (
                ...
                else pl_module.state_dict()
            )
            self._ema_weights = {
                k: p.detach().clone() for k, p in initial_weights.items()
            }
            self._initial_weights = None
